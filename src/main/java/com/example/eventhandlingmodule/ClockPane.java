...
public class ClockPane extends Pane {
    ...
    private void paintClock() {
        ...
        sLine.setStroke(Color.BLUEVIOLET);
        ...
        double hourX = centerX + mLength * Math.sin((hour % 12 + minute / 60.0) * (2 * Math.PI / 60));
        double hourY = centerY + mLength * Math.cos((hour % 12 + minute / 60.0) * (2 * Math.PI / 60));
        Line hLine = new Line(centerX, centerY, hourX, hourY);
        mLine.setStroke(Color.DARKGREEN);
